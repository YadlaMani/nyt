...
import { NotYourTypeContract, BaseBettingTokenContract } from "../utils/thridWebClient";
...
export function MarketBuyInterface({ marketId, market }: MarketBuyInterfaceProps) {
    ...
    const handleConfirm = async () => {
        ...
        // Convert selected option to boolean flags
        ...
        try {
            const tx = await prepareContractCall({
                ...
                method: "function buyShares(uint256 _marketId, bool _isOptionA, bool _isOptionB, bool _isOptionC, bool _isOptionD, uint256 _amount)",
                params: [
                    BigInt(marketId),
                    isOptionA,
                    isOptionB,
    ...
    const getOptionLabel = () => {
        switch (selectedOption) {
            case 'A': return market.optionA;
            case 'B': return market.optionB;
            case 'C': return market.optionC;
            case 'D': return market.optionD;
            default: return '';
        }
    };
    ...
    return (
        <div className="relative transition-[height] duration-200 ease-in-out overflow-hidden" style={{ height: containerHeight }}>
            <div
                ref={contentRef}
                className={cn("w-full transition-all duration-200 ease-in-out", isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4")}
            >
                {!isBuying ? (
                    <div className="grid grid-cols-2 gap-4 mb-4">
                        <Button onClick={() => handleBuy('A')} disabled={!account}>{market.optionA}</Button>
                        <Button onClick={() => handleBuy('B')} disabled={!account}>{market.optionB}</Button>
                        <Button onClick={() => handleBuy('C')} disabled={!account}>{market.optionC}</Button>
                        <Button onClick={() => handleBuy('D')} disabled={!account}>{market.optionD}</Button>
                    </div>
                ) : (
                        ...
                        {buyingStep === 'allowance' ? (
                            <div className="flex flex-col border-2 border-gray-200 rounded-lg p-4">
                                ...
                                <div className="flex justify-end">
                                    ...
                                    <Button onClick={handleCancel} className="ml-2" variant="outline" disabled={isApproving}>Cancel</Button>
                                </div>
                            </div>
                        ) : buyingStep === 'confirm' ? (
                            <div className="flex flex-col border-2 border-gray-200 rounded-lg p-4">
                                ...
                                <p className="mb-4">
                                    You're buying <strong>{amount}</strong> share(s) for option <strong>{getOptionLabel()}</strong>
                                </p>
                                <div className="flex justify-end">
                                    ...
                                    <Button onClick={handleCancel} className="ml-2" variant="outline" disabled={isConfirming}>Cancel</Button>
                                </div>
                            </div>
                        ) : (
